...
public class CommandComplete implements CommandHandler {
    ...
    static final class CompletionContext {
        ...
        static CompletionContext fromTokens(List<String> tokens) {
            String cmd = detectCommand(tokens);

            String last = tokens.isEmpty() ? "" : tokens.get(tokens.size() - 1);
            String prev = tokens.size() >= 2 ? tokens.get(tokens.size() - 2) : "";

            boolean expectNsValue =
                    "--namespace".equalsIgnoreCase(last) || "--ns".equalsIgnoreCase(last) ||
                            "--namespace".equalsIgnoreCase(prev) || "--ns".equalsIgnoreCase(prev);

            return new CompletionContext(tokens, cmd, expectNsValue);
        }
        static String detectCommand(List<String> tokens) {
            if (tokens == null || tokens.isEmpty()) return "";

            // 兼容：可能有程式名，也可能沒有
            // 若 tokens[0] 看起來像程式名，就用 tokens[1] 當 command；否則 tokens[0] 就是 command
            String t0 = tokens.get(0);
            boolean t0LooksLikeProgram =
                    t0.endsWith(".exe") || t0.endsWith(".jar") || t0.contains("\\") || t0.contains("/") || "mytool".equalsIgnoreCase(t0);

            int idx = t0LooksLikeProgram ? 1 : 0;
            return tokens.size() > idx ? tokens.get(idx) : "";
        }
